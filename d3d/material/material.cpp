...
#pragma pack()
    ...
    std::vector<ID3D12Resource*> toonResources(numMaterials);
    ...
    for (size_t i = 0; i < pmd_materials.size(); ++i)
    {
        ...
        sprintf_s(toonFileName, "toon%02d.bmp", pmd_materials[i].toon_index + 1);
    ...
    materialDescriptorHeapDesc.NumDescriptors = numMaterials * 5;
    ...
    for (size_t i = 0; i < numMaterials; ++i)
    {
        ...
        {
            materialSrvDesc.Format = whiteTexture->GetDesc().Format;
            _dev->CreateShaderResourceView(grayGradationTexture, &materialSrvDesc, materialHandle);
        }
        else
        {
            materialSrvDesc.Format = toonResources[i]->GetDesc().Format;
            _dev->CreateShaderResourceView(toonResources[i], &materialSrvDesc, materialHandle);
        }
  ...
  //       worldMatrix *= DirectX::XMMatrixRotationZ(angle);
        ...
        auto matIncSize = _dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV) * 5;
